...
interface HistoryPageMobileProps {
...
export default function HistoryPageMobile({
  history,
  isManageMode,
  onToggleManageMode,
  onDelete,
}: HistoryPageMobileProps) {
      ...
      <div className="container-responsive">
        <div className="flex items-center justify-between mb-4">
          <div>
            ...
            <p className="text-xs text-gray-600">{t('history.description')}</p>
          </div>
          {history.length > 0 && (
            <button
        ...
        {history.length === 0 ? (
          <div className="text-center py-12">
            <p className="text-gray-500 text-sm mb-1">{t('history.noHistory')}</p>
            <p className="text-xs text-gray-400">{t('history.noHistoryDescription')}</p>
          </div>
        ) : (
          <div className="space-y-2.5">
            {history.map((item) => (
              <div key={item.id} className="flex items-center gap-2.5 p-2.5 bg-white border border-gray-200 rounded-lg">
                {isManageMode && (
                  <input
                    type="checkbox"
                ...
                <Link href={`/comic/${item.comic.slug}`} className="flex-shrink-0">
                  <Image src={item.comic.cover_image} alt={item.comic.title} width={50} height={75} className="rounded object-cover" />
                </Link>
                <div className="flex-1 min-w-0">
                  ...
                  {!isManageMode && item.chapter && (
                    <Link href={`/comic/${item.comic.slug}/${item.chapter.slug || `chuong-${item.chapter.chapter_number}-${item.chapter.id}`}`} className="inline-block mt-1.5 px-2.5 py-1 text-xs font-medium text-white bg-blue-600 rounded">
